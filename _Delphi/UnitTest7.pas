...
// -----------------------------------------------------------------------------
//
// Description: Measure INSERTS without RESULT (~ 10sec)
//
// Changes:
...
function TestCase7(ASQLiteServerConnector: TSQLiteServerConnector; ADebug: Boolean): Boolean;
...
begin
  ...
  Table := 'Test7_Table_' + UnitToolsProject.RandomString(10, ['a'..'z','A'..'Z','0'..'9']);
  ...
  while (Result = TRUE) and (WinApi.Windows.GetTickCount-StartTime<10000) do begin
    ...
    if ADebug then Log('SQL: ' + SQL);
    try
      SQLiteResult := ASQLiteServerConnector.ExecSQL(SQL, TRUE);
      Count := Count + 1;
    finally
      ASQLiteServerConnector.FreeResult(SQLiteResult);
    end;
